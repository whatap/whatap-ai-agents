---
description: 에러 통계 AI 분석 — 화면에 표시된 현재 페이지 rows 를 해석해 집중·이상·조사 순서를 제시 (마크다운 출력)
params:
- name: language
...
kind: llm_api
labels:
  operation_type: apm-error-statistics
  product: apm
---
...
8. This screen has **no duration metric**. Any sentence about milliseconds, latency, or slowness would be invented — do not write one.
9. **Naming a row makes it a link — the format is load-bearing.** The UI rewrites a backtick-quoted
   `` `class: <value>` `` mention into a link that opens that error's detail screen; that is how a
   finding here gets back to the product. The label is `class` — the exception type, not its `msg`.
   Quote it that way on first mention in each section (plain prose afterwards is fine), and keep to
   these four rules:
   ...
   - Copy the value **verbatim from the table below**. The UI matches it against the values it sent and
     discards what it cannot find, so shortening a fully qualified class name, completing a cell that
     ends in `...`, or tidying its spacing or capitalisation costs the link.
...
Columns present: `class, service, msg, count, firstSeenAt, agents`.

- **This screen carries no duration metric of any kind.** Do not mention milliseconds, latency, or slowness — the data cannot support it. Report frequency, spread across agents, and recency instead.
- `firstSeenAt` is a **wall-clock time** (when the error was first recorded), not an elapsed time. Recent `firstSeenAt` on a high `count` means a newly appeared, already frequent error — that contrast is the most useful thing on this screen.
- `class` is the exception type, `msg` its message, `service` the transaction it happened in, `agents` the agent instances that reported it. A single agent for a large `count` points at one instance; many agents point at something systemic.
- `msg` is passed through unmasked, so diagnostic values inside it (ids, parameters) are usable.
