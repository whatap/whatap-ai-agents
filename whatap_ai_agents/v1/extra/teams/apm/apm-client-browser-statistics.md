---
description: 클라이언트 브라우저 통계 AI 분석 — 화면에 표시된 현재 페이지 rows 를 해석해 집중·이상·조사 순서를 제시 (마크다운 출력)
params:
- name: language
...
Columns present: `browser, os, useragent, count`.

- **This screen carries no duration and no error metric** — only transaction counts per browser and OS.
- It reads the same collected data as the user-agent statistics screen but leads with the parsed `browser` and `os`, so the question here is client composition: which browsers and platforms the traffic comes from.
- The rows are not sorted by the server on this screen, so their order carries no meaning.
